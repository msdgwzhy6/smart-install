...
const noWrapper = ['button', 'text', 'checkboxs', 'radios', 'pick', 'view', 'scroll', 'tags', 'select']
...
class Region extends Component {
    ...
    _tag = (el, index) => {
        ...
        if(el.type.indexOf('input') === 0) {
            ...
            tag = <Elements.FdText  key={index} item={el}/>
        ...
        if (!util.startWith(el.type, ...noWrapper))
            tag = <View style={[theme.external[el.prop], el.style]} key={index}>
                { tag }
            </View>
    ...
    //是否可以按数组来拆分 比较数据 来进行渲染
    _makeJsx = (columns) => {
        let view = []
        let sub = [] 
        for (let i = 0; i < columns.length; i ++) { 

            // if (util.startWith(columns[i].type, 'input', 'radio')) this.inputFlag = true
            ...
            if (i === columns.length - 1 && util.startWith(columns[i].type, ...['br', 'click', 'backimage', 'swipeout'])) {
                if (util.startWith(columns[i].type, 'br')) {
                    view.push(
                        <View key={i} style={[theme.external[columns[i].type], columns[i].style]}>{ sub }</View>
                    )  
                } else if (util.startWith(columns[i].type, 'backimage')) {
                    this.state.data[columns[i].prop] ? 
                        view.push(
                            <ImageBackground key={i} source={{uri: this.state.data[columns[i].prop]}} style={[theme.external[columns[i].type], columns[i].style]}>
                                { sub } 
                            </ImageBackground>
                        )  :
                        view.push(
                            <ImageBackground key={i} source={columns[i].value} style={[theme.external[columns[i].type], columns[i].style]}>
                                { sub } 
                            </ImageBackground>
                        )  
                    ...
                    view.push( 
                        <TouchableOpacity 
                            key={i} 
                            style={[theme.external[columns[i].type], columns[i].style]} 
                    ...
                    view.push(
                        <Swipeout key={i} right={this._buttons(columns[i].options)}>
                            <View style={[theme.external[columns[i].type], columns[i].style]}>
                                {sub}
                            </View>
                        </Swipeout>
                    )
